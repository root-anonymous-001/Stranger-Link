...
// 🔥 AUTH 100% PROTECTED: base44 hi use ho raha hai 🔥
import { base44 } from "@/api/base44Client";

import { useAuth } from "@/lib/AuthContext";
import { Home, MessageCircle, User, LogOut, ArrowLeft, Bell, Mail, Calendar, Loader2, KeyRound } from "lucide-react";
...
export default function Layout({ children, currentPageName }) {
  ...
  const isInChat = currentPageName === "Chat"; // Chahe session ho ya recent matches
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-500 flex flex-col relative overflow-x-hidden">
      
      {/* Default Blob Background only for NON-CHAT pages */}
      {!isInChat && (
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-3">
              {user ? (
                <div className="flex items-center gap-3">
                  ...
                  <Button variant="ghost" size="icon" className="text-white/40 hover:text-red-400 hover:bg-red-500/10 rounded-2xl transition-all" onClick={() => base44.auth.logout()}>
                    <LogOut className="w-4 h-4" />
                  </Button>
                </div>
              ) : (
                <Button onClick={() => openAuthModal('login')} className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:opacity-90 text-white font-bold border-0 rounded-xl px-6 shadow-lg shadow-purple-500/25">
                  Sign In
                </Button>
              )}
            </div>
            ...
            {!isInChat && (
              <div className="md:hidden flex items-center gap-2">
                {!user && (
                  <Button onClick={() => openAuthModal('login')} size="sm" className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-bold border-0 rounded-xl px-4 text-xs shadow-md shadow-indigo-500/20">
                    Sign In
                  </Button>
                )}
                {user && (
      ...
      <main className="relative pt-16 main-content-mobile flex-1 flex flex-col z-10 w-full h-full">
        {children}
        
        {/* 🔥 AGGRESSIVE GLOBAL AD 🔥 */}
        {currentPageName !== "Home" && (
           <div className="w-full mt-auto mb-[72px] md:mb-0 px-2 py-2 flex justify-center backdrop-blur-sm bg-black/20 border-t border-white/5">
      ...
      {user && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 glass border-t border-white/5" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
          <div className="flex items-stretch">
            {navItems.map((item) => {
              const isActive = currentPageName === item.page;
              return (
                <Link key={item.page} to={createPageUrl(item.page)} className={`flex-1 flex flex-col items-center justify-center gap-1 py-3 transition-all active:scale-95 ${isActive ? "text-white" : "text-white/30"}`}>
                  <div className="relative">
                    {isActive && (
                      <motion.div layoutId="tab-active" className="absolute -inset-2 bg-indigo-500/20 rounded-xl" transition={{ type: "spring", bounce: 0.2, duration: 0.5 }} />
                    )}
                  ...
                  <span className={`text-[10px] font-medium transition-all ${isActive ? "text-indigo-300" : ""}`}>{item.name}</span>
                </Link>
              );
            })}
          </div>
      ...
      <AnimatePresence>
        {isAuthModalOpen && (
           <AuthModal mode={authMode} setMode={(m) => openAuthModal(m)} onClose={closeAuthModal} onSuccess={() => { closeAuthModal(); checkAppState(); }} />
        )}
      </AnimatePresence>
    </div>
  );
...
// ================= GLOBAL AUTH MODAL COMPONENT (WITH base44) =================
function AuthModal({ mode, setMode, onClose, onSuccess }) {
   ...
   const handleGoogleAuth = async () => {
    try {
        ...
        const response = await base44.auth.firebaseLogin({ email: firebaseUser.email, full_name: firebaseUser.displayName });
        if (response) onSuccess(); 
    } catch (error) { alert("Google Login Failed! Check Console."); }
   };

    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} transition={{ type: "spring", bounce: 0.3 }} className="bg-[#111122] border border-white/10 p-6 sm:p-8 rounded-3xl w-full max-w-md shadow-2xl relative overflow-hidden my-auto">
                <button onClick={onClose} className="absolute top-4 right-4 text-white/30 hover:text-white transition-colors bg-white/5 p-2 rounded-full z-10">
                ...
                {/* 🚀 NAYA: 3D Flip Vertical Animation View for Web */}
                <AnimatePresence>
                  {successMsg && (
                    <motion.div 
                      ...
                      className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-xl mb-6 flex flex-col items-center relative z-10"
                    >
                        ...
                        {mode === "register" && (
                            <>
                                <div className="relative">
                                    ...
                                    <input type="email" required readOnly={showOtpInput} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email Address" className={`w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all ${showOtpInput ? "opacity-50 cursor-not-allowed" : ""}`}/>
                                </div>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><User className="h-4 w-4 text-white/30" /></div>
                                    <input type="text" required readOnly={showOtpInput} value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Unique Username" className={`w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all ${showOtpInput ? "opacity-50 cursor-not-allowed" : ""}`}/>
                                </div>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none"><User className="h-4 w-4 text-white/30" /></div>
                                    <input type="text" required readOnly={showOtpInput} value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Display Name" className={`w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all ${showOtpInput ? "opacity-50 cursor-not-allowed" : ""}`}/>
                                </div>
                                <div className="relative">
                                    ...
                                    <input type="date" required readOnly={showOtpInput} value={dob} onChange={(e) => setDob(e.target.value)} className={`w-full pl-11 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all ${showOtpInput ? "opacity-50 cursor-not-allowed" : ""}`} style={{ colorScheme: "dark" }}/>
                                </div>
                            </>
                        )}
                ...
                <button onClick={handleGoogleAuth} type="button" className="w-full mt-6 h-12 rounded-xl bg-white text-black font-bold flex items-center justify-center gap-3 active:scale-95 transition-all hover:bg-gray-100 relative z-10">
                ...
                {/* 🔥 AGGRESSIVE MODAL AD 🔥 */}
                <div className="mt-6 w-full relative z-10 border-t border-white/10 pt-4">
                   <AdSlot type="banner" />
                </div>
            </motion.div>
        </motion.div>
