...
// src/pages/Chat.jsx
import React, { useState, useEffect, useCallback, useRef } from "react";
import { base44, apiClient } from "@/api/base44Client"; // apiClient import kiya
import { useNavigate } from "react-router-dom";
import { createPageUrl } from "@/utils";
import WaitingScreen from "../components/chat/WaitingScreen";
import ChatMessages from "../components/chat/ChatMessages";
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    return () => {
      const s = sessionRef.current;
      if (s?.status === "active" && !s.saved_by_user1 && !s.saved_by_user2) {
        // NAYA: Live API use kar raha hai
  ...
  const loadSession = async (id, me) => {
    ...
    if (s.length > 0) {
      ...
      setMessages(msgs);
      // NAYA: Custom route call through apiClient
  ...
  useEffect(() => {
    ...
    // IMPORTANT FIX: Interval changed from 1000ms to 4000ms to stop backend crash
    const interval = setInterval(async () => {
      try {
        if (sessionId) {
          await loadSession(sessionId, user);
  ...
  const handleDeleteChat = async () => {
    if (window.confirm("Delete this chat permanently?")) {
      await base44.entities.ChatSession.delete(session.id);
      navigate(createPageUrl("Home"));
    }
  };
